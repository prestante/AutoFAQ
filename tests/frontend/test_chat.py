...
@pytest.mark.asyncio
async def test_open_chat(chat_page):
    assert await chat_page.is_chat_open(), "Чат не открыт"
    await chat_page.please_wait()  # for visual assessment
...
async def test_send_message(chat_page):
    ...
    await chat_page.type_message(my_message)
    await chat_page.please_wait()
    await chat_page.send_message()
    await chat_page.please_wait()

    messages = await chat_page.get_chat_messages()
    last_message = await messages[-1].text_content()
    assert my_message == last_message, \
        f"Ожидаем '{my_message}' в качестве последнего сообщения. Получили '{last_message}'"
